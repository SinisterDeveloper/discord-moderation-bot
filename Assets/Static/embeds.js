...
const PunishUserHigher = new MessageEmbed()
	...
	.setDescription(
		'You may not target this member as they have a higher role/same role as you.',
	)
	.setTimestamp();
...
const punishUserHigherBot = new MessageEmbed()
	...
	.setTitle('Permissions missing!')
	.setDescription(
		'I may not target this member as they have a higher role/same role as me.',
	)
	.setTimestamp();
...
const punishNotificationChannel = new MessageEmbed().setColor(
	colors.accentColor,
);
...
function sendUsage(command) {
	...
	usageEmbed.addField(
		'Cooldown',
		`${command.cooldown || defaultCooldown} seconds`,
	);
...
function sendHelp(misc, mod, admin) {
	return new MessageEmbed()
		...
		.addField(
			...
			admin.length ?
				admin.join(', ')
			:	'There are no Administration commands, very strange...',
		)
		.addField(
			`Miscellaneous`,
			misc.length ?
				misc.join(', ')
			:	'There are no Miscellaneous commands, very strange...',
		)
		.addField(
			`Moderation`,
			mod.length ?
				mod.join(', ')
			:	'There are no Moderation commands, very strange...',
		);
}
