	...
	async execute(message, args, client) {
		...
		if (!toMute)
			return message.channel.send({
				content: `Unable to resolve GuildMember \`${args[0]}\``,
			});
		...
		if (
			...
			return message.channel.send({
				embeds: [EMBEDS.moderationCommands.punishUserHigherBot],
			});
		...
		if (
			...
			return message.channel.send({
				embeds: [EMBEDS.moderationCommands.PunishUserHigher],
			});

		if (message.channel.permissionsFor(toMute).has('ADMINISTRATOR'))
			return message.channel.send({
				embeds: [EMBEDS.moderationCommands.isAdmin],
			});

		const reason = args.slice(1).join(' ') || 'No reason specified';

		let muteNotificationDM = EMBEDS.moderationCommands.punishNotificationDM;
		muteNotificationDM.setDescription(
			`You were muted in **${message.guild.name}** for: ${reason}`,
		);

		let punishNotificationChannel =
			EMBEDS.moderationCommands.punishNotificationChannel;
		punishNotificationChannel.setDescription(
			`**${toMute.user.tag}** has been muted for: ${reason}`,
		);
		...
		if (!roleDocs) {
			let mutedRole = await message.guild.roles.create({
				name: 'Muted',
				reason: 'Creating new role to mute users',
			});
			...
			SettingsSchema.findOneAndUpdate(
				...
				.then(() =>
					client.muteRoles.set(message.guild.id, mutedRole.id),
				)
				.catch((err) => {
					console.error(err);
					return message.reply(
						`There was an error while saving modlog to the database:\n\`\`\`js\n${err.message}\`\`\``,
					);
				});
		} else {
